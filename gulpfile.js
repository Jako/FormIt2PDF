...
const bumpCopyright = function () {
    return gulp.src([
        'core/components/formit2pdf/model/formit2pdf/formit2pdf.class.php',
        'core/components/formit2pdf/src/FormIt2PDF.php'
    ], {base: './'})
        .pipe(replace(/Copyright 2022(-\d{4})? by/g, 'Copyright ' + (year > 2022 ? '2022-' : '') + year + ' by'))
        .pipe(replace(/(@copyright .*?) 2022(-\d{4})?/g, '$1 ' + (year > 2022 ? '2022-' : '') + year))
        .pipe(gulp.dest('.'));
};
...
const bumpComposer = function () {
    return gulp.src([
        'core/components/formit2pdf/composer.json',
    ], {base: './'})
